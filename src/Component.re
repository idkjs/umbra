...
let css = ts =>
  Array.reduce(ts, "", (names, (name, pass)) =>
    switch (pass, names) {
    | (true, "") => name
...
module Link = {
  ...
  let make = (~children, ~path, ~className="") =>
    <a href="#" className onClick={navigate(path)}> children </a>;
};
    ...
    let classNames =
      Array.map(classes, name =>
        ...
        | ("hint", _) => (
            ...
            | (Setup({fealty, selected: Some(_)}), Field({vector})) =>
              Board.inBounds(fealty, vector)
            | (
                Setup({selected: Some(Corps({status: Marshalled(a)}))}),
                Corps({status: Marshalled(b)}),
              ) =>
              !Vector.eq(a, b)
            | (Setup({selected: Some(_)}), Corps({status: Marshalled(_)})) =>
              true
            | _ => false
            },
          )
...
module Control = {
  ...
  let make = (~children, ~event: State.event, ~enabled: bool) => {
    ...
    <button disabled={!enabled} onClick={_ => dispatch(event)}>
      children
    </button>;
  };
};
...
module Panel = {
  ...
  let make = () => {
    ...
    let camp =
      switch (state) {
      | Setup({fealty, camps})
      | Playing({fealty, camps}) => Camps.get(camps, fealty)
      | _ => []
      };
...
module Board = {
  ...
  let make = () => {
    ...
    switch (state) {
    | Setup({board, fealty: player, camps})
    | Playing({board, fealty: player, camps}) =>
      let yours = Camps.get(camps, player);
      let friendly =
        Array.map(List.toArray(yours), rank =>
          <Piece tile={Corps({rank, fealty: player, status: Camped})} />
        );

      let theirs = Camps.get(camps, Fealty.opposing(player));
      let opponent =
        Array.map(List.toArray(theirs), rank =>
          <Piece
            tile={
              Corps({rank, fealty: Fealty.opposing(player), status: Camped})
            }
          />
        );
      ...
      <div>
        ...
        <div className="board">
          {React.array(Array.map(board, tile => <Piece tile />))}
        </div>
        <div className="pieces">
          {React.array(
             [@ns.ternary]
             (
               if (player == Fealty.Red) {
                 friendly;
               } else {
                 opponent;
               }
...
module Root = {
  ...
  let make = () => {
    let (state, dispatch) =
      React.useReducer(State.resolve, State.NotStarted);
